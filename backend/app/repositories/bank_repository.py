...
class BankRepository:
    ...
    def list_accounts_by_institution(self, institution_id: str) -> List[Account]:
        return (
    ...
    def aggregate_account_summary(self, account_id: str) -> Optional[dict]:
        income = (
            self.db.query(func.coalesce(func.sum(Transaction.amount), 0))
            .filter(Transaction.account_id == account_id, Transaction.type == "income")
            .scalar()
        )
        expenses = (
            self.db.query(func.coalesce(func.sum(Transaction.amount), 0))
            .filter(Transaction.account_id == account_id, Transaction.type == "expense")
            .scalar()
        )
        ...
        return {
            ...
            "balance": Decimal((income or 0) - (expenses or 0)),
